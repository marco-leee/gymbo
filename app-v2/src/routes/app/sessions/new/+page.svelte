<script lang="ts">
	...
	function handleSubmit() {
		...
		const valid = payloadExercises.filter(
						...
						<Select.Trigger class="w-full">
							...
							<span class={clientId ? "" : "text-muted-foreground"}>
								{clientId && clientsQuery.data?.clients
									? (clientsQuery.data.clients.find((c) => c.id === clientId)
											?.full_name ?? "Select client")
									: "Select client"}
							</span>
						</Select.Trigger>
						<Select.Content>
				...
				<div class="space-y-3">
					<div>
						...
						<p class="text-muted-foreground text-xs">
							Add exercises to this session (optional).
						</p>
					</div>
					<div class="space-y-2">
						{#each exercises as row, index (index)}
							<Collapsible.Root
								open={exercises.length === 1 || index === exercises.length - 1}
							>
								<Card>
									<CardHeader
									...
									<Collapsible.Content>
										<CardContent class="space-y-4 pt-0">
											...
											<div class="grid gap-4 sm:grid-cols-2">
												<div class="space-y-2">
													...
													<Select.Root
														...
														onValueChange={(v) =>
															(row.type = (v ??
																"strength") as ExerciseRow["type"])}
													>
														<Select.Trigger class="w-full">
															<span>{row.type}</span>
														</Select.Trigger>
														<Select.Content>
															<Select.Item value="strength"
																>Strength</Select.Item
															>
															<Select.Item value="cardio">Cardio</Select.Item>
															<Select.Item value="flexibility"
																>Flexibility</Select.Item
															>
														</Select.Content>
													</Select.Root>
												</div>
												<div class="space-y-2">
													...
													<Select.Root
														...
														onValueChange={(v) =>
															(row.measurement = (v ??
																"reps") as ExerciseRow["measurement"])}
													>
														<Select.Trigger class="w-full">
															<span
																>{row.measurement === "reps"
											...
											{:else}
												<div class="space-y-2">
													<Label for="ex-target-duration-{index}"
														>Target duration (sec)</Label
													>
													<Input
