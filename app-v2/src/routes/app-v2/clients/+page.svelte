...
<div class="flex flex-col gap-8">
	<div class="flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="app-v2-display text-4xl md:text-5xl" style="color: var(--app-v2-text);">Clients</h1>
			<p class="mt-1 max-w-xl text-sm" style="color: var(--app-v2-muted);">
				Search and open a client. Add new clients in one tap.
			</p>
		</div>
		<Button href="/app-v2/clients/new" class="app-v2-cta min-h-12 rounded-lg px-6 text-base">
			<PlusIcon class="mr-2 h-5 w-5" aria-hidden="true" />
			New client
		</Button>
	</div>

	<div class="app-v2-card flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
	...
	{:else if clientsQuery.data && clientsQuery.data.clients.length > 0}
		<ul class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each clientsQuery.data.clients as client (client.id)}
				<li class="app-v2-card flex flex-col gap-3 p-5">
					<div>
						<p class="text-lg font-semibold leading-tight">{client.full_name}</p>
						<p class="mt-1 text-sm" style="color: var(--app-v2-muted);">{client.email}</p>
					</div>
					<p class="text-sm" style="color: var(--app-v2-muted);">
						Added {formatDate(client.created_at)}
					</p>
					<Button
						class="app-v2-cta mt-auto min-h-11 w-full rounded-lg"
	...
	{:else}
		<div class="app-v2-card py-16 text-center">
			<p style="color: var(--app-v2-muted);">No clients match. Create your first one.</p>
			<Button href="/app-v2/clients/new" class="app-v2-cta mt-4 min-h-11">New client</Button>
		</div>
	{/if}
...
<Sheet.Root bind:open={drawerOpen}>
	<Sheet.Content
		...
		// style="background: var(--app-v2-surface-2); color: var(--app-v2-text); border-color: var(--app-v2-border);"
	>
		<Sheet.Header>
			<Sheet.Title>Client details</Sheet.Title>
			<Sheet.Description style="color: var(--app-v2-muted);">
		...
		<div class="py-6">
			{#if clientDetailQuery.isLoading}
				<p style="color: var(--app-v2-muted);">Loading client details…</p>
			...
			{:else if clientDetailQuery.data}
				...
				<div class="space-y-4">
					<div>
						<p class="text-sm" style="color: var(--app-v2-muted);">Full name</p>
						<p class="font-medium">{client.full_name}</p>
					</div>
					<div>
						<p class="text-sm" style="color: var(--app-v2-muted);">Email</p>
						<p class="font-medium">{client.email}</p>
					</div>
					<div>
						<p class="text-sm" style="color: var(--app-v2-muted);">Gender</p>
						<p class="font-medium">{client.gender || 'Not specified'}</p>
					</div>
					<div class="grid grid-cols-2 gap-4">
						<div>
							<p class="text-sm" style="color: var(--app-v2-muted);">Height</p>
							<p class="font-medium">{client.height_cm > 0 ? `${client.height_cm} cm` : 'Not specified'}</p>
						</div>
						<div>
							<p class="text-sm" style="color: var(--app-v2-muted);">Weight</p>
							<p class="font-medium">{client.weight_kg > 0 ? `${client.weight_kg} kg` : 'Not specified'}</p>
						</div>
					</div>
					<div>
						<p class="text-sm" style="color: var(--app-v2-muted);">Added on</p>
