<script lang="ts">
	...
	const sessionsQuery = createQuery(() => {
		...
		return {
			queryKey: ['sessions', 'app-v2', selectedClientId, selectedStatus, datePreset],
			queryFn: () =>
				listSessions({
	...
	function hubUrl(sessionId: string, view: 'session' | 'analysis') {
		return `/app-v2/sessions/${sessionId}?view=${view}`;
	}
	...
	function primaryRowAction(session: Session): { label: string; href: string } {
		...
		if (session.status === 'in-progress') {
			return { label: 'Continue', href: `/app-v2/sessions/${session.id}/run` };
		}
...
<div class="flex flex-col gap-8">
	<div class="flex flex-wrap items-end justify-between gap-4">
		<div>
			<h1 class="app-v2-display text-4xl md:text-5xl" style="color: var(--app-v2-text);">Sessions</h1>
			<p class="mt-1 max-w-xl text-sm" style="color: var(--app-v2-muted);">
				Bold layout — jump in with one tap. Live workouts use execution mode.
			</p>
		</div>
		<Button href="/app-v2/sessions/new" class="app-v2-cta min-h-12 rounded-lg px-6 text-base">
			<PlusIcon class="mr-2 h-5 w-5" aria-hidden="true" />
			New session
		</Button>
	</div>

	<div class="app-v2-card flex flex-wrap items-center gap-3 p-4">
		<div class="flex items-center gap-2">
			<SearchIcon class="h-4 w-4 shrink-0" style="color: var(--app-v2-muted);" aria-hidden="true" />
		...
		<div class="flex items-center gap-2">
			<CalendarIcon class="h-4 w-4 shrink-0" style="color: var(--app-v2-muted);" aria-hidden="true" />
			<div class="flex flex-wrap gap-1 rounded-lg border p-1" style="border-color: var(--app-v2-border);">
				{#each ['today', 'week', 'month', 'all'] as preset}
					<Button
						variant={datePreset === preset ? 'default' : 'ghost'}
						size="sm"
						class="min-h-9 rounded-md {datePreset === preset ? 'app-v2-cta' : 'app-v2-ghost'}"
		...
		{#if selectedClientId || selectedStatus || datePreset !== 'today'}
			<Button variant="ghost" size="sm" class="app-v2-ghost min-h-9" onclick={() => {
				selectedClientId = '';
				selectedStatus = '';
				datePreset = 'today';
			}}>
	...
	{#if sessionsQuery.isLoading}
		<p style="color: var(--app-v2-muted);">Loading…</p>
	...
	{:else if sessionsQuery.data && sessionsQuery.data.sessions.length > 0}
		<ul class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			{#each sessionsQuery.data.sessions as session (session.id)}
				...
				<li class="app-v2-card flex flex-col gap-4 p-5">
					<div class="flex items-start justify-between gap-2">
						<div>
							<p class="app-v2-display text-2xl leading-none" style="color: var(--app-v2-accent);">
								{formatTime(session.scheduled_at)}
							</p>
							<p class="mt-1 text-sm" style="color: var(--app-v2-muted);">
					...
					<p class="text-sm" style="color: var(--app-v2-muted);">
						{getExerciseCount(session)} exercise{getExerciseCount(session) === 1 ? '' : 's'}
					</p>
					<div class="mt-auto flex flex-wrap items-center gap-2">
						<Button href={act.href} class="app-v2-cta min-h-11 flex-1 rounded-lg">
							{act.label}
						</Button>
						<DropdownMenu.Root>
							<DropdownMenu.Trigger
								class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-v2-accent)]"
							...
							<DropdownMenu.Content align="end">
								...
								{#if session.status === 'scheduled' || session.status === 'in-progress'}
									<DropdownMenu.Item onclick={() => goto(`/app-v2/sessions/${session.id}/run`)}>
										<VideoIcon class="mr-2 h-4 w-4" />
										Execution
									</DropdownMenu.Item>
									<DropdownMenu.Item onclick={() => goto(`/app-v2/sessions/${session.id}/record`)}>
										Table recorder
									</DropdownMenu.Item>
								{/if}
								{#if session.status === 'completed'}
	...
	{:else}
		<div class="app-v2-card py-16 text-center">
			<p style="color: var(--app-v2-muted);">No sessions match. Create one to start.</p>
			<Button href="/app-v2/sessions/new" class="app-v2-cta mt-4">New session</Button>
		</div>
	{/if}
