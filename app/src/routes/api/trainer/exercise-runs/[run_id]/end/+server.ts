...
import { endTrainerRun } from '$lib/server/trainer-worker';
...
export const POST: RequestHandler = async (event) => {
	try {
		...
		await endTrainerRun(runId);
		const updated = await updateCoachedExerciseRun(runId, {
			status: 'ended',
			ended_at: new Date()
		});
		...
		throw error(500, 'Failed to end exercise run');
	}
};
