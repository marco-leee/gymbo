...
import { getCoachedExerciseRun, listSafetyEvents } from '$lib/server/trainer-runs';
...
export const GET: RequestHandler = async (event) => {
	try {
		...
		const events = await listSafetyEvents(runId);
		return json({ events });
	} catch (err) {
		if (isHttpError(err)) throw err;
		throw error(500, 'Failed to list safety events');
	}
};
