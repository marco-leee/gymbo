<script lang="ts">
	...
	const createMutationState = createMutation(() => ({
		...
		onSuccess: (session) => {
			goto(`/app/sessions/v2/${session.id}?view=overview`);
		}
	}));
...
<div class="flex flex-1 flex-col gap-4 p-4 pt-0">
	<div class="flex items-center gap-2">
		<Button href="/app/sessions/v2" variant="ghost" size="icon" aria-label="Back to sessions">
		...
		<Card class="w-full max-w-6xl">
			...
			<CardContent class="space-y-4">
				...
				<div class="flex flex-wrap gap-2 pt-4">
					...
					<Button href="/app/sessions/v2" variant="outline" class="min-h-11">Cancel</Button>
				</div>
			</CardContent>
		</Card>
	</div>
